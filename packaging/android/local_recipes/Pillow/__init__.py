...
class PillowRecipe(PyProjectRecipe):
    ...
    This recipe lets us build Pillow with support for different image and font
    types. To use some of Pillow's features, we must build some libraries. By
    default we automatically trigger the build of the libraries below::
    ...
    You can also enable extra image types by requesting libraries via the
    ...
    opt_depends = ['libwebp']
    # OSpRad local patch (see local_recipes/numpy/__init__.py for why). Pillow's own
    # [build-system] requires is just setuptools>=77, already above.
    ...
    def get_recipe_env(self, arch, **kwargs):
        ...
        # flags to build the Pillow recipe successfully, so we add them here.
        ...
        # libwebp is an optional dependency, so we add the flags if it is in
        # our `ctx.recipe_build_order`.
